...
parser.add_argument('--settings', metavar='DIR', default='/mnt/c/Users/k2i12/Desktop/set2',
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_freq == 0:
            prec1,map_score = validate(val_loader, model, criterion)
...
def validate(val_loader, model, criterion):
    ...
    # 모든 배치의 output과 target을 하나의 텐서로 연결
    all_outputs = torch.cat(all_outputs)
    all_targets = torch.cat(all_targets)

    # 정확도 계산
    num_classes = all_outputs.size(1)  # 클래스의 수 계산
    all_targets_onehot = torch.nn.functional.one_hot(all_targets, num_classes=num_classes)  # 타겟을 원핫 인코딩
    map_score = calculate_map(all_targets_onehot, all_outputs, num_classes)  # mAP 계산

    # 최종 결과 출력
    print(' * Prec@1 {top1.avg:.3f} Prec@3 {top3.avg:.3f} mAP {map_score:.3f}'.format(
        top1=top1, top3=top3, map_score=map_score))

    return top1.avg, map_score
...
def calculate_map(y_true, y_pred, num_classes):
    y_true_np = y_true.cpu().numpy()  # 타겟을 numpy 배열로 변환
    y_pred_np = y_pred.cpu().numpy()  # 출력값을 numpy 배열로 변환

    ap_list = []

    # 각 클래스별로 average precision 계산
    for class_idx in range(num_classes):
        ...
        ap = metrics.average_precision_score(true_class, pred_class)
        ap_list.append(ap)
