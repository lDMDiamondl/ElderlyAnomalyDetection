...
2. **Two‑Stream 행동 분류기**: 공간(stream RGB) + 시간(stream Optical Flow) 두 입력을 사용하여 행동을 분류합니다. 원본 구현은 ResNet을 사용합니다.
3. **후처리**:  원본 영상에 예측 라벨을 오버레이한 결과 비디오를 생성할 수 있습니다.

> ⚠️ **주의사항**: 개인 PC에서 사용하던 코드를 기반으로 하여 일부 경로(path) 지정이 다소 지저분하거나 절대경로가 포함되어 있을 수 있습니다. 사용 환경에 따라 경로를 수정해 사용하는 것을 권장합니다.
