...
def VideoSpatialPrediction_cam(
        rgb_frames,
        net,
        num_categories=6,
        start_frame=0,
    ...
    for i in range(num_samples):
        # 프레임 리스트에서 numpy 배열 형태의 이미지 선택
        img = rgb_frames[min(i * step, duration - 1)]
        if img is None:
            print(f"Warning: Missing image at index {i}")
            continue
